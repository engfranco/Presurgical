...
##
##
##  Update: 27/01/2013 - Added despiking
##########################################################################################################################
...
echo ---------------------------------------
echo ---- PREPROCESSING FUNCTIONAL SCAN ${subj} ${epi} ----
echo ---------------------------------------
...
3dcalc \
-a ${study}${subj}.${epi}.nii.gz'['${TRstart}'..$]' \
-expr 'a' \
-prefix ${study}${subj}.${epi}_dr.nii.gz

## 0.5 - Despiking first 
3dDespike -prefix ${study}${subj}.${epi}_dspk.nii.gz ${study}${subj}.${epi}_dr.nii.gz

## 1. Slice time correction
3dTshift -tpattern alt+z -prefix ${study}${subj}.${epi}_STC.nii.gz ${study}${subj}.${epi}_dspk.nii.gz


##2. Deoblique
echo "Deobliquing ${subj}"
3drefit -deoblique ${study}${subj}.${epi}_STC.nii.gz

##3. Reorient into fsl friendly space (what AFNI calls RPI)
echo "Reorienting ${subj}"
3dresample -orient RPI -inset ${study}${subj}.${epi}_STC.nii.gz -prefix ${study}${subj}.${epi}_ro.nii.gz



##4. Motion correct to first good image of timeseries
...
##6. the first good image (without T1 eq effects) for use in registration
echo "Getting example_func for registration for ${subj}"
3dcalc -a ${study}${subj}.${epi}_ss.nii.gz'[0]' \
-expr 'a' \
