...
#### Parameters that typically don't need to be set for each patient
#####
set anat_dicom_NAME = AXI3DFSPGRBRAVO_
set task1_name = MOT_M  # Motor Mao
set task1_dicom_NAME = FMRIMOTOR_
set task2_name = MOT_P  # Motor Pé
set task2_dicom_NAME = FMRIMOTORPE_
set task3_name = VIS  # visão
set task3_dicom_NAME = XXX
...
## Get subjects to run
	...
	if (${motor_mao_run}) then
		${scripts_dir}/conv_dicom_create_directories.sh ${study} ${subj} ${subjdirectory} ${task1_name} ${task1_dicom_NAME}
	endif
	...
	#9-I  Pseudo palavras 1
	if (${pseudo_run}) then
		${scripts_dir}/conv_dicom_create_directories.sh ${study} ${subj} ${subjdirectory} ${task9I_name} ${task9I_dicom_NAME}
	endif

	#9-II  Pseudo palavras 2
	if (${pseudo_run}) then
		${scripts_dir}/conv_dicom_create_directories.sh ${study} ${subj} ${subjdirectory} ${task9II_name} ${task9II_dicom_NAME}
	endif

	#10  Listening
	if (${list_run}) then
		${scripts_dir}/conv_dicom_create_directories.sh ${study} ${subj} ${subjdirectory} ${task10_name} ${task10_dicom_NAME}
	endif
